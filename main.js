...
function omega(LOL) {
  ...
  TESTOWANIEFLICKA = setInterval(function () {
    if (!ZALADOWANE) {
      var msgs = [
        "Looping through Instagram servers...",
        "Safely connecting with IG_" +
          chance.integer({ min: 3, max: 333 }) +
          ".DB()...",
        "Looking for " + LOL + " user...",
        "Downloading Instagram profile data...",
      ];
...
function addfl() {
  ...
  if (FFF > 1) {
    if (!TRYBZAPASOWY) {
      ...
      var fl = new CountUp(
        "followers-c",
        parseInt(rawfollowers),
        parseInt(rawfollowers) +
          parseInt($(".ui.dropdown.followers").dropdown("get value")),
        0,
        5,
        options
      );
      ...
      $(".ig-extras").each(function () {
        ...
        var like = new CountUp(
          ajdi,
          parseInt($(this).text()),
          parseInt($(this).text()) +
            parseInt($(".ui.dropdown.likes").dropdown("get value")),
          0,
          5,
          options
        );
      ...
      $(".NATA").fadeOut(1000, function () {
        $("#igback").fadeIn(1000, function () {
              ...
              if (aktualnyprogres < 100) {
                ...
                if (BACKSKONCZONE) {
                  $(".choosehowmany").fadeOut("slow", function () {
                    $(".botlike").html(
                      "<strong>ERROR! Botlike behaviour detected.</strong> Complete human verifications to complete changes and receive followers and likes."
                    );
          ...
          var loa = [
            "Connecting with Instagram followers/likes servers...",
            "Changing " + LUL + " followers/likes values... ",
            "Changing currentFollowers_ to +" +
              parseInt($(".ui.dropdown.followers").dropdown("get value")) +
              " followers...",
            "Changing eachPhotoLikes_ to +" +
              parseInt($(".ui.dropdown.likes").dropdown("get value")) +
              " likes...",
            "Succesfully added " +
              parseInt($(".ui.dropdown.likes").dropdown("get value")) +
